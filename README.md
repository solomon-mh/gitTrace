...
**Brand:** the mark is a hex badge framing a traced path — three waypoints
(the commits) growing in size as the trace climbs toward the head. Commit
history as a signal you follow from origin to HEAD. Type: Chakra Petch for the
wordmark and headings (angular, cut-corner, reads like a schematic label),
Inter for UI, JetBrains Mono for anything numeric. Sky-to-green gradient on
the trace only. Dark by default.

![gitTrace dashboard](docs/screenshot.png)
...
> gitTrace never hides repos by visibility — private repos appear whenever the
...
src/
  ...
  components/
    Logo.tsx                     brand mark (hex badge + traced waypoints)
    dashboard/
...
GitHub gives you 5,000 GraphQL points/hour. gitTrace keeps well under that:
